...
from app.services.provider_factory import model_registry, provider_factory
...
def list_all_models():
    ...
    try:
        ...
        return jsonify(
            {"models": models_dict, "total": sum(len(models) for models in models_dict.values())}
        ), 200
...
def get_model_info(provider_name: str, model_id: str):
    ...
    try:
        ...
        if not model_info:
            return jsonify(
                {"error": f"Model '{model_id}' not found in provider '{provider_name}'"}
            ), 404
